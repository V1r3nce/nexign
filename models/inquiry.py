...
def prepare_inquiries(
    ...
    :param activation_date: Дата активации или список дат активации основного продукта.
    :param additional_product_activation_date: Дата активации или список дат активации доп. продукта.
    ...
    :param individualized_subs_fee: Стоимость или список стоимостей продуктов в заявке которые необходимо индивидуализировать
    :param additional_individualized_subs_fee: Стоимость или список стоимостей доп. продуктов в заявке которые необходимо индивидуализировать
    ...
    Example:
            ...
            prepare_inquiries(["mobile"], product_offering_id=[500067], activation_date=datetime, as_list=False) - продажа одного продукта с указанием даты активации
            prepare_inquiries(["mobile", "mobile"], product_offering_id=[500067, 500067], activation_date=[datetime, datetime], as_list=False) - продажа одной заявки с двумя продуктами с указанием даты активации у каждого
            prepare_inquiries(["internet"], "additional_product=["+2 ГБ"], additional_product_activation_date=datetime, as_list=False) - продажа одного продукта с указанием даты активации
            prepare_inquiries(["satellite_sale"], product_offering_id=[500055], additional_product=[["Корпоративный доступ к VPN(L3)", "Белый IP"]], additional_product_activation_date=[[datetime, datetime]], as_list=False) - продажа в одной заявке одного продукта и двух доп. продуктов, для доп. продуктов задана дата активации.
    """
    ...
    def generate_none_list_for_empty_entity(entity: Any, entity_name: str) -> list:
        if not entity:
            ...
            check_that(
                ...
                ValueError,
                f"Список категорий и список '{entity_name}' должны быть одинаковой длины.",
            )
        return entity

    def create_entity_list_or_empty_list(entity: Any, entity_name: str) -> list:
        if entity is not None:
            entity = entity if isinstance(entity, list) else [entity]
        else:
            entity = generate_none_list_for_empty_entity(entity, entity_name)
        return entity
    ...
    additional_product_activation_date = (
        [additional_product_activation_date]
        if isinstance(additional_product_activation_date, datetime)
        else additional_product_activation_date
    )
    ...
    individualized_subs_fee = generate_none_list_for_empty_entity(
        individualized_subs_fee, "продукты для индивидуализации"
    )
    additional_product = generate_none_list_for_empty_entity(additional_product, "дополнительные продукты")
    additional_product_activation_date = generate_none_list_for_empty_entity(
        additional_product_activation_date, "даты активации дополнительных продуктов"
    )
    additional_individualized_subs_fee = generate_none_list_for_empty_entity(
        additional_individualized_subs_fee, "доп. продукты для индивидуализации"
    )
    ...
    if as_list:
        ...
        for (
            ...
            add_subs_fee_list,
            add_product_activation_date_list,
        ) in zip(
            category,
            product_offering_id,
            individualized_subs_fee,
            ...
            product.activation_date = activ_date_list

            add_product_name_list = (
                add_product_name_list if isinstance(add_product_name_list, list) else [add_product_name_list]
            )
            add_subs_fee_list = create_entity_list_or_empty_list(add_subs_fee_list, "доп. продукты для индивидуализации")
            add_product_activation_date_list = create_entity_list_or_empty_list(
                add_product_activation_date_list, "даты активации доп. продуктов"
            )

            additional_products = [
                AdditionalProduct(
                    product_name=add_product_name,
                    individualized_subs_fee=add_subs_fee,
                    activation_date=add_product_activation_date,
                )
                for add_product_name, add_subs_fee, add_product_activation_date in zip(
                    add_product_name_list, add_subs_fee_list, add_product_activation_date_list
                )
