...
class BaseLisElements:
    def __init__(self) -> None:
        ...
        self.SAVE_BUTTON = Element(".n-popup ps-button[on-submit*='updatePhoneNumber()']", "Кнопка 'Сохранить'")
        self.MASS_SAVE_BUTTON = Element(".n-popup ps-button[on-submit*='massUpdatePhoneNumber']", "Кнопка 'Сохранить'")
        self.CANCEL_BUTTON = Element(".n-popup ps-button[icon*='block']", "Кнопка 'Отменить'")
        ...
        self.MODAL_FIRST_BTN = ElementsList("div.n-popup ps-button:first-child", "Первая кнопка модального окна")
        self.MODAL_SECOND_BTN = ElementsList("div.n-popup ps-button:last-child", "Вторая кнопка модального окна")
        ...
        self.TABLE_FIRST_COLUMN_ELEMENTS = ElementsList(
            "div.n-popup tbody tr td:nth-child(1)", "Элементы первой колонки таблицы модального окна"
        )
        self.REFRESH_MODAL_TABLE_BTN = Element("div.n-popup [ng-click*='refreshGrid']", "Кнопка 'Обновить данные'")

        # Шаблоны
        self.CHOOSE_SEARCH_TEMPLATE_BTN = Element(
            "//div[contains(@ng-click, 'loadTemplates()')][2]", "Кнопка 'Выбрать шаблон поиска'"
        )
        self.TEMPLATE_OPTIONS = ElementsList(
            "[ng-repeat*='item in templates.data'][ng-click]", "Варианты шаблонов поиска'"
        )
        self.SAVE_SEARCH_TEMPLATE_BTN = Element(
            "//div[contains(@ng-click, 'loadTemplates()')][1]", "Кнопка 'Сохранить шаблон поиска'"
        )
        self.NEW_TEMPLATE_BTN = Element("//span[text()='Новый шаблон']/parent::div", "Кнопка 'Новый шаблон'")
        self.REMOVE_TEMPLATE_BTN = Element(
            "[ng-click*='dialogs.deleteTemplate.open']", "Кнопка 'Удалить текущий шаблон'"
        )
        ...
        self.NEW_TEMPLATE_NAME_INPUT = Element(
            "[ng-model*='dialogs.addTemplate.templateName']", "Поле ввода названия шаблона"
        )
        self.TEMPLATE_SAVE_BTN = Element(
            "[on-submit*='dialogs.addTemplate.addNewTemplate']", "Кнопка 'Сохранить' шаблон"
        )
        self.TEMPLATE_CANCEL_BTN = Element(
            "[ng-click*='dialogs.addTemplate.close']", "Кнопка 'Отменить' создание шаблона"
        )
        ...
        self.COMMUTATOR_SUBMIT_BTN = Element("[on-submit*='commutatorDialog.submit']", "Кнопка 'Выбрать'")

        # Модальное окно Изготовление SIM-карт/Создание предсвязок
        self.QUANTITY_INPUT_CREATE_SIM = Element(
            "[ng-model*='localModel.params.count']", "Поле ввода 'Количество штук в заказе'"
        )
        self.START_RANGE_INPUT_CREATE_SIM = Element(
            "[ng-model*='simcardRangeParams.startIMSI']", "Поле ввода IMSI 'Начальное значение'"
        )
        self.END_RANGE_INPUT_CREATE_SIM = Element(
            "[ng-model*='simcardRangeParams.endIMSI']", "Поле ввода IMSI 'Конечное значение'"
        )
        self.NEXT_BTN = Element("[ng-click*='next']", "Кнопка 'Далее'")
        self.CANCEL_BTN = Element("[ng-click*='master.show=false']", "Кнопка 'Отменить'")
        self.FORM_BTN = Element("[on-submit*='createTask()']", "Кнопка 'Сформировать/Аннулировать'")
        ...
        self.NUMBER_TYPE_FIELD = Element("[ng-click*='loadPhoneNumberTypes'] > div:nth-child(1)", "Поле 'Тип нумерации'")
        ...
        self.USE_GOAL_FIELD = Element("[ng-click*='loadPhoneNumberPurpose'] > div > div", "Поле 'Цель использования'")
        self.TAKE_FREE_AFTER_INPUT = Element(
            "[ng-model*='phoneNumberRangeParams.freeMonths']", "Поле 'Брать освобожденные после'"
        )
        self.TEMPLATE_INPUT = Element("[ng-model*='phoneNumberRangeParams.patternMSISDN']", "Поле 'Шаблон'")
        self.START_MSISDN_INPUT = Element(
            "[ng-model*='phoneNumberRangeParams.startMSISDN']", "Поле 'Начальное значение'"
        )
        self.END_MSISDN_INPUT = Element("[ng-model*='phoneNumberRangeParams.endMSISDN']", "Поле 'Конечное значение'")
        ...
        self.OPERATION_DETAIL_TITLE = Element(
            "//*[@ng-if='model.views.details.show']/div/span[1]", "Заголовок 'Подробности операции'"
        )
        self.COMPLETE_PERCENT = Element(
            "//*[@ng-if='model.views.details.show']/div/span[2]", "Процент выполнения задания"
        )
        self.OPERATION_DETAIL_IMSI_LIST = ElementsList(
            "[rows*='model.tasksItems.data'] tbody tr td:nth-child(2)", "Список IMSI"
        )
        self.OPERATION_DETAIL_STATUS_LIST = ElementsList(
            "[rows*='model.tasksItems.data'] tbody tr td:nth-child(9)", "Список Состояние обработки"
        )
