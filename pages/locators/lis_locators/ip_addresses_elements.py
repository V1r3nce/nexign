...
class IpAdressesElementsLis(BaseElementsLis):
    ...
    def __init__(self, page: Page):
        ...
        self.IP_OPTION_INTERVAL = Element(
            "//ps-list-item[contains(@user-value, 'INTERVAL')]", "Опция фильтра 'IP-адрес' По диапазону", self.page
        )
        self.IP_OPTION_VALUE = Element(
            "//ps-list-item[contains(@user-value,'VALUE')]", "Опция фильтра 'IP-адрес' Точное значение", self.page
        )
        self.IP_SELECTED_OPTION = Element("#ID71001 div span", "Выбранное значение 'IP-адрес'", self.page)
        self.IP_START_VALUE = Element(
            "#ID71001 input[ng-model='model.searchParameters.IPAddress.startValue']", "Начальное значение IP", self.page
        )
        self.IP_END_VALUE = Element(
            "#ID71001 input[ng-model='model.searchParameters.IPAddress.endValue']", "Конечное значение IP", self.page
        )

        self.IP_TYPE_FILTER_BTN = Element(
            "#ID71003 .b-combobox__buttons", "Кнопка открыть фильтр 'Тип IP-адреса'", self.page
        )
        ...
        self.IP_TYPE_SELECTED_OPTION = Element(
            "#ID71003 div span .b-multiselect-item__title", "Выбранное значение 'Типа IP-адреса'", self.page
        )
        ...
        self.IP_STATUS_SELECTED_OPTION = Element(
            "#ID71002 div span .b-multiselect-item__title", "Выбранное значение 'Типа IP-адреса'", self.page
        )
        ...
        self.TEMPLATE_NAMES_LIST = ElementsList(
            "[ng-click='selectedTemplate(item)'] span", "Список наименований шаблонов поиска", self.page
        )
        ...
        self.TEMPLATE_OPTION_NEW = Element(
            "//ps-list-item[contains(@user-value,'new')]", "Опция сохранения шаблона поиска 'Новый шаблон'", self.page
        )
        ...
        self.ADD_ADDRESS_MODAL = Element(
            "form[name='ipUploadForm']", "Модальное окно 'Добавление IP-адресов'", self.page
        )
        ...
        self.ACCESS_POINT_FIELD = Element(
            "form[name='ipUploadForm'] [ng-model='apnDialog.accessPointName'] input", "Поле 'Точка доступа'", self.page
        )
        ...
        self.ADD_IP_BTN = Element(
            "form[name='ipUploadForm'] .n-popup-foot ps-button[icon='ok']", "Кнопка 'Добавить'", self.page
        )

        # ACCESS_POINT_SELECTION_MODAL
        self.ACCESS_POINT_MODAL = Element(
            "div[ui-lock='apnDialog.apnLock']", "Модальное окно 'Выбор точки доступа APN'", self.page
        )
        self.ACCESS_POINT_OPTION = Element(
            "div[ui-lock='apnDialog.apnLock'] tbody > tr:first-child", "Точка доступа", self.page
        )
        self.ACCESS_POINT_NAME = Element(
            "[ui-lock='apnDialog.apnLock'] tr:first-child > td:first-child div", "Наименование точки доступа", self.page
        )
        self.CHOOSE_BTN = Element("div[ui-lock='apnDialog.apnLock'] ps-button[icon='ok']", "Кнопка 'Выбрать'", self.page)

        # CONFIRMATION_IP_MODAL
        self.CONFIRMATION_IP_MSG = Element(
            ".ps-dialog.n-popup .n-popup-message-align", "Текст подтверждения ip", self.page
        )

        # INFORMATION_IP_MODAL
        self.INFORMATION_OK_BTN = Element(
            "body .ps-dialog[tabindex='-1']:nth-last-of-type(2) ps-button", "Кнопка 'ОК'", self.page
        )
        ...
        self.TEMPLATE_SAVE_BTN = Element(
            ".ps-dialog[tabindex='-1']:nth-last-of-type(2) ps-button:nth-child(1)", "Кнопка 'Сохранить'", self.page
        )
