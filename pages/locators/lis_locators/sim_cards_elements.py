...
class SimCardElementsLis(BaseElementsLis):
    ...
    def __init__(self, page: Page):
        ...
        self.REFRESH_BTN = Element("[user-value*='simSearch'] [ng-click*='searchSim']", "Кнопка 'Обновить'", self.page)
        self.HISTORY_BTN = Element(
            "[user-value*='simSearch'] [ng-click*='historyDialog.open']", "Кнопка 'История'", self.page
        )
        ...
        self.CHOOSE_COMMUTATOR_BTN = Element(
            "[user-value*='simSearch'] [ng-click*='commutatorDialog.show']", "Кнопка 'Задать коммутатор'", self.page
        )
        ...
        self.DOWNLOAD_BTN = Element(
            "[user-value*='simSearch'] [ng-click*='dialogs.csvExport.export']", "Кнопка 'Выгрузить в Excel'", self.page
        )
        self.SEARCH_BTN = Element("[user-value*='simSearch'] a.lis-toolbar-search__link", "Кнопка 'Поиск'", self.page)
        ...
        self.STATE_FILTER_OPTIONS = ElementsList(
            "//*[@user-value='item.SIMCardStateId']", "Опции фильтр 'Состояние'", self.page
        )
        ...
        self.EXPIRATION_DATE_INPUT = Element(
            "[value*='model.simCards.filter.expirationDate'] input", "Поле ввода 'Срок действия'", self.page
        )
        ...
        self.CHOSEN_COMMUTATOR_INPUT = Element(
            "[ng-model*='model.simCards.filter.equipmentId'] input", "Поле 'Коммутатор'", self.page
        )
        ...
        self.LINK_POOL_INPUT = Element("[ng-model*='tariffTemplates.model'] input", "Поле 'Набор связывания'", self.page)
        self.MAP_INPUT = Element(
        ...
        self.AGENT_INPUT = Element("[ng-model*='model.simCards.filter.agent'] input", "Поле 'Дилер'", self.page)
        self.TARIFF_INPUT = Element("[ng-model*='filter.ratePlanIds'] input", "Поле 'Тарифный план'", self.page)
        ...
        self.FILTER_SEARCH_BTN = Element("[ng-click*='searchSim'][icon='search']", "Кнопка 'Найти'", self.page)
        ...
        self.MSISDN_HEADER = Element(
            "[user-value*='simSearch'] tr th.n-grid__title:nth-child(3)", "Заголовок/Кнопка 'MSISDN'", self.page
        )
        ...
        self.LINE_CHECKBOXES = ElementsList(
            "[user-value*='simSearch'] tr.n-grid__row span.n-check-checkbox", "Чекбоксы строк таблицы", self.page
        )
        self.IMSI_NUMBERS = ElementsList(
            "[user-value*='simSearch'] tr.n-grid__row td:nth-child(3)", "Номера IMSI", self.page
        )
        self.NUMBERS_STATUSES = ElementsList(
            "[user-value*='simSearch'] tr.n-grid__row td:nth-child(7)", "Статусы номеров", self.page
        )
        self.NUMBERS_STATES = ElementsList(
            "[user-value*='simSearch'] tr.n-grid__row td:nth-child(9)", "Состояния номеров", self.page
        )
        self.NUMBERS_BLOCK_STATUS = ElementsList(
            "[user-value*='simSearch'] tr.n-grid__row td:nth-child(11)", "Статус блокировки номеров", self.page
        )
        self.NUMBERS_COMMUTATOR = ElementsList(
            "[user-value*='simSearch'] tr.n-grid__row td:nth-child(12)", "Коммутатор номеров", self.page
        )
        self.EXPIRATIONS_DATES = ElementsList(
            "[user-value*='simSearch'] tr.n-grid__row td:nth-child(14)", "Сроки действия", self.page
        )
        ...
        self.MODAL_EXPIRATION_DATE_INPUT = Element(
            "[value*='dialogs.periodChange.expirationDate'] input", "Поле ввода 'Изменение срока действия'", self.page
        )
        ...
        self.MODAL_CHOSEN_INPUT = Element(
            "div.n-popup [ng-required*='states.required']", "Поле выбранный дилер", self.page
        )
        self.MODAL_OPEN_SELLER_LIST = Element(
            "div.n-popup ps-button[ng-if*='options.isDropButton']", "Кнопка открыть список дилеров", self.page
        )
        ...
        self.MODAL_SEND_BTN = Element(
            "[ng-click*='dialogs.simTransferToDealer.transfer']", "Кнопка 'Передать'", self.page
        )
        self.MODAL_CANCEL_BTN = Element(
            "[icon='block'][ng-click*='dialogs.simTransferToDealer']", "Кнопка 'Отменить'", self.page
        )
        ...
        self.CHANGE_COMMUTATOR_BTN = Element(
            "[user-value*='simUploads'] [ng-click*='commutatorDialog.show']", "Кнопка 'Задать коммутатор'", self.page
        )
        self.PROJECT_CHANGE_BTN = Element(
            "[user-value*='simUploads'] [ng-click*='showProjectChangeDialog']", "Кнопка 'Изменить проект'", self.page
        )
        self.PERIOD_CHANGE_BTN = Element(
            "[user-value*='simUploads'] [ng-click*='dialogs.periodChange']", "Кнопка 'Изменить срок действия'", self.page
        )
        ...
        self.IMSI_HEADER_UPLOAD_SIMS = Element(
            "[user-value*='simUploads'] tr th.n-grid__title:nth-child(3)", "Заголовок/Кнопка 'IMSI'", self.page
        )
        self.LINE_CHECKBOXES_UPLOAD_SIMS = ElementsList(
            "[user-value*='simUploads'] tr.n-grid__row span.n-check-checkbox", "Чекбоксы строк таблицы", self.page
        )
        self.IMSI_NUMBERS_UPLOAD_SIMS = ElementsList(
            "[id*='uploadsGrid'] tr.n-grid__row td:nth-child(3)", "Номера IMSI", self.page
        )
        self.ICC_NUMBERS_UPLOAD_SIMS = ElementsList(
            "[id*='uploadsGrid'] tr.n-grid__row td:nth-child(4)", "Номера ICC", self.page
        )
        self.EXPIRATIONS_DATE_UPLOAD_SIMS = ElementsList(
            "[id*='uploadsGrid'] tr.n-grid__row td:nth-child(16)", "Поля Дата окончания действия", self.page
        )
        self.COMMUTATORS_UPLOAD_SIMS = ElementsList(
            "[id*='uploadsGrid'] tr.n-grid__row td:nth-child(18)", "Поля Коммутатор", self.page
        )
        self.PROJECTS_UPLOAD_SIMS = ElementsList(
            "[id*='uploadsGrid'] tr.n-grid__row td:nth-child(19)", "Поля Проект", self.page
        )
        ...
        self.CHOSEN_PROJECT_ADD_SIM_MODAL = Element(
            "[ng-init*='importSimDialog.loadSimProjects'] div div", "Поле 'Проект'", self.page
        )
        ...
        self.TEMPLATE_INPUT_ADD_SIM_MODAL = Element(
            "[ng-model*='templateId.loadSIMCardTemplateId'] input", "Поле Шаблон", self.page
        )
        self.EXPIRATION_DATE_INPUT_ADD_SIM_MODAL = Element(
            "[ng-model*='selectedValues.expirationDate'] input", "Поле Срок действия", self.page
        )
        self.ADD_BUTTON_ADD_SIM_MODAL = Element(
            "form[name='importSimForm'] ps-button[on-submit*='submit']", "Кнопка 'Добавить'", self.page
        )
        self.CANCEL_BTN_ADD_SIM_MODAL = Element(
            "ps-button[ng-click*='importSimDialog.cancel(importSimForm)']", "Кнопка 'Отменить'", self.page
        )

        # Модальное окно 'Изменить проект'
        self.PROJECT_OPTIONS_CHANGE_PROJECT_MODAL = ElementsList(
            "//ps-list-item[@user-value='value.SIMCardProjectId']", "Опции проектов", self.page
        )
        ...
        self.SAVE_BUTTON_PROJECT_MODAL = Element(
            "ps-button[on-submit*='massActions.changeProject']", "Кнопка 'Сохранить'", self.page
        )
        self.CANCEL_BTN_PROJECT_MODAL = Element(
            "ps-button[ng-click*='dialogs.projectChange.isOpened=false']", "Кнопка 'Отменить'", self.page
        )

        # Модальное окно 'Устанавливаемый срок действия'
        self.NEW_DATE_INPUT_MODAL = Element(
            "[value*='model.dialogs.expirationDate'] input", "Поле ввода 'Устанавливаемый срок действия'", self.page
        )
        self.SAVE_BTN_DATE_MODAL = Element(
            "ps-button[on-submit*='assActions.changeExpirationDate']", "Кнопка 'Сохранить'", self.page
        )
        self.CANCEL_BTN_DATE_MODAL = Element(
            "ps-button[ng-click*='dialogs.periodChange.isOpened=false']", "Кнопка 'Отменить'", self.page
        )
