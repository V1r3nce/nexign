...
class RegistryElements(DynamicElements):
    ...
    def __init__(self, page: Page):
        ...
        self.REFRESH_BTN = Element(
            "//div[contains(@class, 'platform-table')]/div/div[1]/button[1]", "Кнопка 'Обновить'", self.page
        )
        ...
        self.PAYMENT_DATES_FIELDS = ElementsList(
            "[class*=table-tbody] [class*=table-row] > div:nth-child(1)", "Поля 'Дата платежа'", self.page
        )
        self.STATUS_FIELDS = ElementsList(
            "[class*=table-tbody] [class*=table-row] > div:nth-child(2)", "Поля 'Статус'", self.page
        )
        self.CHECK_NUM_FIELDS = ElementsList(
            "[class*=table-tbody] [class*=table-row] > div:nth-child(3) span", "Поля 'Номер чека'", self.page
        )
        self.CHECK_SUM_FIELDS = ElementsList(
            "[class*=table-tbody] [class*=table-row] > div:nth-child(4)", "Поля 'Сумма чека'", self.page
        )
        self.PAYMENT_SUM_FIELDS = ElementsList(
            "[class*=table-tbody] [class*=table-row] > div:nth-child(5)", "Поля 'Сумма платежа'", self.page
        )
...
class RegistryDetailsElements(DynamicElements):
    ...
    def __init__(self, page: Page):
        ...
        self.FORM_TITLE = Element("div[class*=drawer-title] h3", "Заголовок формы", self.page)
