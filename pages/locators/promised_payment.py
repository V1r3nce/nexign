...
class PromisedPaymentPage(BaseElements):
    ...
    def __init__(self, page: Page):
        ...
        self.AN_CANCEL_BTN = Element('(//div[contains(@class, "platform-root-scrollable-container")]//button)[4]', "Кнопка 'Аннулировать'",
                             self.page)
        self.PROMISED_PAYMENT_EL = ElementsList(".ant-table-tbody tr", "Обещанный платеж из таблицы",
                                                self.page)
        ...
        self.COMMENT_FLD = Element(".ant-modal-body textarea", "Поле ввода комментария",
                                   self.page)
        self.STATUS_HISTORY_BTN = Element('(//div[contains(@class, "platform-root-scrollable-container")]//button)[5]', "Кнопка 'История статусов'",
                                          self.page)
        self.STATUS_PAYMENTS_FORM = Element('.ant-drawer-content-wrapper', "Форма со статусами обещанного платежа",
                                            self.page)
        self.CHARACTERISTICS_BTN = Element('(//div[contains(@class, "platform-root-scrollable-container")]//button)[7]', "Кнопка 'Шестеренка",
                                     self.page)
        self.CHARACTERISTICS_FLD = ElementsList('.ant-dropdown-placement-bottomRight [type="checkbox"]', "Настраиваемая характеристика",
                                                self.page)
        self.CHARACTERISTICS_FORM_BTN = ElementsList('.ant-dropdown-placement-bottomRight button', "Кнопки в поле характеристик",
                                                     self.page)

    @allure.step("Выбрать настраиваемые характиристики")
    def choose_characteristics(self):
