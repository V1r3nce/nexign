...
class SystemProblems(DynamicForms):
    ...
    def __init__(self, page: Page):
        ...
        self.REVIEW_PROCESS_BEFORE = Element(".ant-collapse-content-box .platform-grid-item:nth-child(1)", "Обработать до", self.page)
        self.REVIEW_PRIORITY = Element(".ant-collapse-content-box .platform-grid-item:nth-child(2)", "Приоритет", self.page)
        self.REVIEW_CREATION_DATE = Element(".ant-collapse-content-box .platform-grid-item:nth-child(3)", "Дата создания", self.page)
        self.REVIEW_REGISTERED = Element(".ant-collapse-content-box .platform-grid-item:nth-child(4)", "Зарегистрировал", self.page)
        self.REVIEW_PLANNED_END_DATE = Element(".ant-collapse-content-box .platform-grid-item:nth-child(5)", "Дата закрытия (план)", self.page)
        self.REVIEW_FACT_END_DATE = Element(".ant-collapse-content-box .platform-grid-item:nth-child(6)", "Дата закрытия (факт)", self.page)
        self.REVIEW_ORIGIN_DATE = Element(".ant-collapse-content-box .platform-grid-item:nth-child(7)", "Дата возникновения", self.page)
        ...
        self.HISTORY_PLANNED_END_DATE = Element("((//div[contains(@id, 'panel-history')]//div[contains(@class, 'platform-scrollable')])[2] //div[contains(@class, 'platform-grid-item')])[1]", "Дата завершения (план)", self.page)
        self.HISTORY_END_DATE = Element("((//div[contains(@id, 'panel-history')]//div[contains(@class, 'platform-scrollable')])[2] //div[contains(@class, 'platform-grid-item')])[2]", "Дата завершения (факт)", self.page)
        self.HISTORY_QUEUE = Element("((//div[contains(@id, 'panel-history')]//div[contains(@class, 'platform-scrollable')])[2] //div[contains(@class, 'platform-grid-item')])[3]", "Очередь", self.page)
        self.HISTORY_DURATION = Element("((//div[contains(@id, 'panel-history')]//div[contains(@class, 'platform-scrollable')])[2] //div[contains(@class, 'platform-grid-item')])[4]", "Продолжительность", self.page)
