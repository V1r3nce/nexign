...
class TestPreparingManyIPAddresses:
    ...
    def test_preparing_many_ip_addresses(self, base_url: str) -> None:
        ...
        with allure.step('Выбрать точку доступа в списке и нажмите на кнопку "Выбрать"'):
            ...
            delay(0.3, reason="Кнопке нужно время даже после того, как она стала доступной")
        ...
        with allure.step(
            'В блоке "Диапазон IP-адресов" ввести уникальные IP-адрес в поля "Начальное значение" и "Конечное значение" (превосходящее предыдущее) и нажать "Добавить"'
        ):
