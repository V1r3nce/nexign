...
class TestUploadingListIPAddresses:
    ...
    @allure.id(583575)
    def test_uploading_list_ip_addresses(self, page: Page, base_url: str, remove_file_from_download_folder):
        ...
        with allure.step('Выбрать все IP-адреса с помощью чекбокса и нажать на кнопку "Выгрузить в файл"'):
            ...
            self.file_check.check_excel_file_group_of_fields_contains(fields=[[i,1] for i in range(1, 16)],
                                                                  expected_values=ip_list)
