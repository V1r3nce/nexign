...
class TestSimCardsShipments:
    ...
    @allure.tag("can_auth", "success")
    def test_sim_shipment_to_test_seller_by_imsi_from_file(self, api_request_auth_context: APIRequestContext,
        ...
        ship_sims_file_path = self.sim_shipment_lis.create_csv_file_to_upload_sim_shipment(file_shipment_name,
        ...
        (self.sim_shipment_lis.sims_shipment_elements.IMSI_START_INPUT.
         check_attribute_by_value("placeholder", "из файла"))
        ...
        (self.sim_shipment_lis.sims_shipment_elements.OPERATION_DETAIL_IMSI_LIST[0].
         wait_to_have_text(str(last_sims_imsi + 1)))
        (self.sim_shipment_lis.sims_shipment_elements.OPERATION_DETAIL_IMSI_LIST[1].
         wait_to_have_text(str(last_sims_imsi + 2)))
    ...
    @allure.tag("can_auth", "success")
    def test_sim_shipment_to_main_warehouse_by_imsi_from_file(self, api_request_auth_context: APIRequestContext,
                                                              remove_file_from_download_folder):
        sim_requests = SimCardsRequests(api_request_auth_context)
    ...
    @allure.description("Перемещение SIM-карт в Отдел обслуживания и тестовому дилеру (По списку IMSI из файла."
                        " Неверный файл)")
    @allure.tag("can_auth", "success")
    def test_sim_shipment_to_test_seller_by_imsi_from_wrong_file(self, api_request_auth_context: APIRequestContext,
                                                                 remove_file_from_download_folder):
        file_name = "wrong_file_ship_sims.csv"
        wrong_file_path = (self.sim_cards_page.
                           create_txt_file_to_upload_sim(file_name,
                                                         ["1" * 15, "2" * 15],
                                                         ["1" * 17, "2" * 17]))
    ...
    @allure.tag("can_auth", "success")
    def test_sim_shipment_to_main_warehouse_by_imsi_from_wrong_file(self, api_request_auth_context: APIRequestContext,
                                                                    remove_file_from_download_folder):
        file_name = "wrong_file_ship_sims.csv"
        wrong_file_path = (self.sim_cards_page.
                           create_txt_file_to_upload_sim(file_name,
