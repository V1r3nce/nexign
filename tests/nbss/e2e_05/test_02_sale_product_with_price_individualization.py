...
class TestSaleProductWithPriceIndividualization:
    ...
    @allure.title("02. Продажа продуктового предложения с индивидуализацией стоимости абонентской платы")
    @allure.id(652744)
    @allure.description(
        """
        Проверить возможность продажи продуктового предложения
        с индивидуализацией стоимости абонентской платы.
        """
    )
    def test_sale_product_with_individual_subscription_price(self) -> None:
        ...
        with allure.step("Шаг 1: Открытие формы редактирования цены абонентской платы"):
