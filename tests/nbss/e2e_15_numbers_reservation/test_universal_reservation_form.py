...
class TestUniversalReservationForm:
    ...
    def setup(self, nexign_stand_login, create_switch, create_number_and_start_exploitation) -> None:
    ...
    def test_mobile_phone_reservation_view_selected_numbers(self, create_organization: OrganizationClient) -> None:
        ...
        self.inquiries_page.search_number_by_mask("%7")
        check_that(
            lambda: all(number.endswith("7") for number in self.reserve_form.NUMBER.text_list),
        ...
        self.reserve_form.TITLE.not_to_be_visible()
    ...
    @allure.id(654971)
    @pytest.mark.parametrize("create_switch", [DefaultStandardId.mobile], indirect=True)
    def test_mobile_phone_reservation_change_number_class(self, create_organization: OrganizationClient) -> None:
        self.base_page.open(f"{BASE_URL}customer-hierarchy-management/customers/{test_context.client.user_id}/overview")
        self.inquiries_page.sale_initialization(
            test_context.client, create_add_agreement="manual", need_contact_data=True, priority="Высокий", add_kp="no"
        )
        ...
        check_that(
            lambda: all(item.text == "Платиновый" for item in self.reserve_form.NUMBER_CLASS_NAME),
            exception=NexignBaseException,
            message="Не найдены номера с классом номера = 'Платиновый'",
        )
    ...
    def test_sim_card_reservation(self, create_organization: OrganizationClient) -> None:
        ...
        self.inquiries_page.search_number_by_mask("%5")
        check_that(
            lambda: all(number.endswith("5") for number in self.reserve_form.NUMBER.text_list),
            exception=NexignBaseException,
            message="Мобильные номера, оканчивающиеся на 5 не найдены",
        )
