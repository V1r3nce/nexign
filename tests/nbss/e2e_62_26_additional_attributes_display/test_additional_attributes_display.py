...
class TestAdditionalAttributesDisplay:
    ...
    def setup(self, nexign_stand_login, create_organization_with_agreement_and_account: OrganizationClient) -> None:
        ...
        self.inquiries_sale_card_page = InquirySaleCardPage()
    ...
    def test_sale_card_additional_attributes(self) -> None:
        ...
        with allure.step("Перейти на вкладку 'Карточка продажи'"):
            ...
            self.inquiries_sale_card_page.check_attribute_groups(
                displayed=[DESCRIPTION_GROUP, AGREEMENT_AND_ACCOUNT_GROUP]
            )
    ...
    def test_resource_replacement_additional_attributes(self) -> None:
        ...
        with allure.step("Проверить состав коллапсов на вкладке 'Обзор'"):
            ...
            self.inquiries_sale_card_page.check_attribute_groups(displayed=[DESCRIPTION_GROUP, REPLACEMENT_GROUPS])
    ...
    def test_test_attributes_inquiry_additional_attributes(self) -> None:
        ...
        with allure.step("Нажать кнопку 'Редактировать'"):
            self.inquiries_sale_card_page.inquiry_sale_card.ATTRIBUTES_EDIT_BTN.click()
            ...
            self.inquiries_sale_card_page.inquiry_sale_card.ATTRIBUTES_AGREEMENT.wait_to_be_visible()
            delay(0.5, "Ожидание открытия блока редактирования после клика")

        with allure.step("Изменить описание, заменить договор1 и ЛС1 на договор2 и ЛС2, сохранить"):
            new_agreement = f"{self.client.agreements[1].number} от {get_current_datetime_string(is_full_format=False)}"
            new_account = self.client.agreements[1].account.number
            self.inquiries_sale_card_page.inquiry_sale_card.ATTRIBUTES_AGREEMENT.select_by_value(new_agreement)
            self.inquiries_sale_card_page.inquiry_sale_card.ATTRIBUTES_ACCOUNT.to_have_value("")
            self.inquiries_page.locators.LOAD_SPINS.wait_to_be_visible()
            self.inquiries_page.locators.LOAD_SPINS.wait_not_to_be_visible()
            self.inquiries_sale_card_page.inquiry_sale_card.ATTRIBUTES_ACCOUNT.select_by_value(new_account)
            self.inquiries_sale_card_page.inquiry_sale_card.ATTRIBUTES_SAVE_BTN.click()

        with allure.step("Проверить, что поля недоступны для редактирования и значения изменились"):
            self.inquiries_sale_card_page.inquiry_sale_card.ATTRIBUTES_EDIT_BTN.wait_to_be_visible(timeout=15000)
    ...
    def test_sale_collapses(self) -> None:
        ...
        with allure.step("Перейти на 'Карточку продажи'"):
            self.inquiries_page.click_tab(InquiryTab.SaleCard)
            self.inquiries_sale_card_page.check_attribute_groups(displayed=[DESCRIPTION_GROUP])

        with allure.step("Завершить продажу"):
            self.inquiries_page.click_tab(InquiryTab.ActiveStep)
            self.inquiries_page.click_next(InquiryStep.SaleCompletion, timeout=240000)
