...
class TestEditProductActivationDateAfterSaleFinishPastTime:
    ...
    def setup(
        ...
        self.yesterday_activation_date = get_shifted_datetime_string(shift="-1d", is_full_format=False)
        self.allowed_activation_end_date = (datetime.now() + relativedelta(months=3)).date().isoformat()

    @allure.title("08. Изменение даты активации продукта на прошедшее время")
    @allure.id(914010)
    def test_edit_product_activation_date_after_sale_finish_past_time(self) -> None:
        ...
        self.client_product_profile.edit_product_activation_date_form.ACTIVATION_DATE.fill(
            self.yesterday_activation_date
        )
