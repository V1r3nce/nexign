...
class TestIndividualCustomerCreate:
    ...
    def test_individual_customer_create(self, base_url: str) -> None:
        ...
        with allure.step("Переход в профиль клиента и проверка данных"):
            ...
            self.client_profile.GENDER.to_have_value(client.gender)
        ...
        with allure.step("Ищем клиента"):
            ...
            self.home_page.search_client(inn=client.inn, customer_status="Действующий")
    ...
    def test_individual_customer_create_only_required_fields(self, base_url: str) -> None:
        self.home_page.create_customer_with_type("individual", only_required_fields=True)
        client = test_context.client

        with allure.step("Перейти в профиль клиента и проверить данные"):
        ...
        with allure.step("Ищем клиента"):
            ...
            self.client_search_page.FOUNDED_CLIENTS.not_to_be_visible()
            self.home_page.search_client(customer_name=client.sur_name, customer_status="Действующий")
            self.client_search_page.FOUNDED_CLIENTS.wait_to_be_visible()
    ...
    def test_individual_customer_create_document_out_of_date(self, base_url: str) -> None:
        ...
        with allure.step("В открывшейся форме пользователь вводит данные клиента"):
            self.customer_create_form.fill_data_for_individual_client(
                self.user, only_required_fields=True, need_second_page=False
            )
    ...
    def test_create_individual_customer_from_process_sale(self, base_url: str) -> None:
        ...
        with allure.step("В открывшейся форме пользователь вводит данные клиента"):
            self.home_page.create_customer_with_type("individual", with_initialization=False)
            client = test_context.client

            self.create_request_form.CLIENT.wait_to_be_visible(timeout=30000)
            self.create_request_form.CLIENT.to_contain_text(client.first_name, timeout_sec=15)
