...
@allure.link(
    url="confluence.nexign.com/pages/viewpage.action?pageId=462935916", name="LLD Прием и аннулирование платежа"
)
...
class TestManageBankPayments:
    ...
    def test_payments_relocate_balance(
        ...
        doc_number = generate_random_number(4)
        today_user_friendly_view = get_current_datetime_string(is_full_format=False)
        client_info = self.personal_account_api.create_agreement_and_account(client_info)
        first_account_id = client_info.agreements[0].accounts[0].id
        first_account_num = str(client_info.agreements[0].accounts[0].number)
        second_account_num = str(client_info.agreements[1].accounts[0].number)

        payment_data = PaymentUniblpInfo(amount=payment_amount, account_id=first_account_id, document_number=doc_number)
        self.payment_api_uniblp.wait_check_create_payment(payment_data)
        self.payment_api_uniblp.create_payment(payment_data)
