...
from common.helpers.time_helpers import (
    delay,
...
class TestManageBankPayments:
    ...
    def test_cancel_bank_payment_payments_form(
        ...
        self.payment_page.payment_elements.CHECK_NUM_FIELDS.to_contain_text(0, str(doc_number_1))
        ...
        self.payment_page.payment_elements.SUM_FIELDS.to_contain_text(0, f"{payment_data_1.amount}.00")
        ...
        self.payment_page.payment_elements.CHECK_NUM_FIELDS.wait_to_be_visible()
        self.payment_page.payment_elements.SUM_FIELDS[0].wait_to_have_text(f"{payment_data_1.amount}.00")
        ...
        (
            self.payment_details_elements.SUBTITLE.wait_to_have_text(
                re.compile(f"На сумму {payment_data_1.amount}.00 от {today_user_friendly_view}")
            )
        )
        ...
        self.payment_details_elements.PAYMENT_DETAILS[2].to_contain_text(str(doc_number_1))
        self.payment_details_elements.PAYMENT_DETAILS[3].to_contain_text(f"{payment_data_1.amount}.00")
        self.payment_details_elements.PAYMENT_DETAILS[4].wait_to_have_text(
            re.compile(rf"{payment_data_1.amount}.00\sRUB")
        )
    ...
    def test_cancel_bank_payment_expired_period(self, base_url: str, create_user_with_agreement_and_account):
        ...
        current_date = get_current_moscow_datetime().strftime("%d.%m.%Y")
        payment_amount = generate_random_number(3)
        doc_number = generate_random_number(4)
        payment_data = PaymentUniblpInfo(
        ...
        self.payment_page.fill_registry_date(old_date_user_friendly_view, current_date)
