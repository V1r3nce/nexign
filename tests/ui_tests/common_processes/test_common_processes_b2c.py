...
class TestCommonBusinessProcessesB2C:
    ...
    def test_b2c_sale(self, base_url: str, create_user: int) -> None:
        ...
        with allure.step("Создание продажи"):
            ...
            self.inquiries_page.ADDED_PRODUCT.wait_to_have_count(1, timeout=20000)
            ...
            self.product_edit_form.auto_reserve_phone_number_resources()
    ...
    def test_turn_off_pp(self, base_url: str, create_user: int) -> None:
        ...
        with allure.step("Проверка вкладки 'Элементы заказа'"):
            self.inquiries_page.TABS.wait_to_be_visible()
            self.inquiries_page.TABS[1].click()
            self.inquiries_page.TABS[1].check_attribute_by_value("aria-selected", "true")
            self.inquiries_page.PRODUCTS_NAME[0].wait_to_have_text(product.product_name)
            self.inquiries_page.PRODUCTS_STATUS.wait_to_have_text("Отключение")

        self.inquiries_page.TABS[0].click()
        self.inquiries_page.TABS[0].check_attribute_by_value("aria-selected", "true")
