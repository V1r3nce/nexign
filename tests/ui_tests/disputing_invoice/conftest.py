...
def create_client_with_billing_and_claim(
    ...
    with allure.step(f"Добавление платежа для ЛС: {client.account_id}"):
        ...
        wait_that(
            lambda: payment_api.get_payments(client.account_id, "-paymentDate").json()["items"][0]["status"]["code"]
            == "SUCCEEDED",
            exception=CreatePaymentException,
            timeout=25,
            sleep_seconds=0.5,
    ...
    with allure.step(f"Проведение биллинга для ЛС: {client.account_id}"):
        ...
        wait_that(
            lambda: len(
                billing_api.get_billing_profile_runs(
                    billing_profile_id,
            ...
            timeout=10,
            sleep_seconds=0.5,
            message="Биллинг не появился в указанное время",
        )
        wait_that(
