...
class TestViewBillingDiscount:
    ...
    @allure.id(676533)
    def test_view_filter_billing_discount(self, create_individual_user: IndividualClient, base_url) -> None:
    ...
    @allure.id(676536)
    def test_clear_filter_billing_discount(self, create_individual_user: IndividualClient, base_url) -> None:
        individual_user = create_individual_user
        client, product = self.client_request_api.product_sale(
            individual_user.user_id,
        )
    ...
    @allure.id(676558)
    def test_billing_discount_products_view(self, create_individual_user: IndividualClient, base_url) -> None:
        individual_user = create_individual_user
        client, product = self.client_request_api.product_sale(
            individual_user.user_id,
        )
        ...
        with allure.step("Проверяем продукт, к которому применена скидка"):
            ...
            self.discount_page.locators.PRODUCTS[0].wait_to_have_text(product.product_name)

    @allure.title("12. Просмотр абонентов, к которым применена скидка")
    @allure.id(676636)
    def test_billing_discount_accounts_view(self, create_individual_user: IndividualClient, base_url) -> None:
        individual_user = create_individual_user
        client, product = self.client_request_api.product_sale(
            individual_user.user_id,
        )
        ...
        with allure.step("Проверяем абонента, к которому применена скидка"):
            ...
            self.discount_page.locators.SUBSCRIBERS[0].to_contain_text(str(product.subs_id))

    @allure.title("16. Просмотр условий применимости")
    @allure.id(676640)
    def test_billing_discount_conditions_view(self, create_individual_user: IndividualClient, base_url) -> None:
        individual_user = create_individual_user
        client, product = self.client_request_api.product_sale(
            individual_user.user_id,
        )
