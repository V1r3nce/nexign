...
class TestAccrualAdjustment:
    ...
    def setup(
        ...
        add_two_msisdn_free_and_open_for_use: tuple[str, str],
    ...
    def test_create_negative_adjustment_bill(self, base_url: str) -> None:
        ...
        with allure.step("Выбрать нужный счет, запомнить значения полей 'Начислено' и 'Доначислено'"):
            ...
            self.billing_accounts.locators.BILLING_PROPERTIES.wait_for_text_in_all(["Начислено"])
            property_index = self.billing_accounts.locators.BILLING_PROPERTIES.text_list.index("Начислено")
            charged = float(self.billing_accounts.locators.BILLING_PROPERTY_VALUES[property_index].text)
            self.billing_accounts.locators.BILLING_PROPERTIES.wait_for_text_in_all(["Доначислено"])
            property_index = self.billing_accounts.locators.BILLING_PROPERTIES.text_list.index("Доначислено")
            charged_additionally = float(self.billing_accounts.locators.BILLING_PROPERTY_VALUES[property_index].text)

        with allure.step("Перейти на вкладку 'Детали', запомнить значение поля 'Откорректированно'"):
            self.billing_accounts.locators.DETAILS_TAB.click()
            self.billing_accounts.locators.DETAIL.wait_to_be_visible()
            adjusted = float(self.billing_accounts.locators.DETAIL_ADJUSTED[0].text)
        ...
        with allure.step("Проверить отображение суммы корректировки на вкладке 'Свойства'"):
            self.billing_accounts.locators.BILLING_PROPERTIES.wait_for_text_in_all(["Доначислено"])
            property_index = self.billing_accounts.locators.BILLING_PROPERTIES.text_list.index("Доначислено")
            self.billing_accounts.locators.BILLING_PROPERTY_VALUES[property_index].wait_to_have_text(
                f"{charged_additionally + adjustment_sum:.2f}"
            )

        with allure.step("Перейти на вкладку 'Детали', проверить что сумма корректировки учтена"):
            self.billing_accounts.locators.DETAILS_TAB.click()
            self.billing_accounts.locators.DETAIL.wait_to_be_visible()
            self.billing_accounts.locators.DETAIL_ADJUSTED[0].wait_to_have_text(f"{adjusted + adjustment_sum:.2f}")
    ...
    def test_create_negative_adjustment_bill_with_summ_more_then_accrual(self, base_url: str) -> None:
        ...
        with allure.step("Выбрать нужный счет, запомнить значение поля 'Начислено'"):
            self.billing_accounts.locators.ACCOUNT_NUMS_LIST.wait_to_be_visible()
            self.billing_accounts.locators.ACCOUNT_NUMS_LIST.click(0)
            self.billing_accounts.locators.BILLING_PROPERTIES.wait_for_text_in_all(["Начислено"])
            property_index = self.billing_accounts.locators.BILLING_PROPERTIES.text_list.index("Начислено")
    ...
    def test_create_negative_adjustment_tax_invoice(self, base_url: str) -> None:
        ...
        with allure.step("Перейти на вкладку 'Детали', запомнить значение поля 'Откорректированно'"):
            self.billing_accounts.locators.DETAILS_TAB.click()
            self.billing_accounts.locators.DETAIL.wait_to_be_visible()
            detail_adjusted = float(self.billing_accounts.locators.DETAIL_ADJUSTED[0].text)

        with allure.step("Перейти на вкладку 'Счета-фактуры', запомнить значение поля 'Откорректированно'"):
            tax_invoice_type = "Счет-фактура на начисления"
            self.billing_accounts.locators.INVOICES_TAB.click()
            self.billing_accounts.locators.INVOICE.wait_to_be_visible()
            self.billing_accounts.locators.INVOICE_TYPE.wait_for_text_in_all([tax_invoice_type])
            tax_invoice_index = self.billing_accounts.locators.INVOICE_TYPE.text_list.index(tax_invoice_type)
            tax_invoice_adjusted = float(self.billing_accounts.locators.INVOICE_ADJUSTED[tax_invoice_index].text)
        ...
        with allure.step("Перейти на вкладку 'Детали', проверить что сумма корректировки учтена"):
            self.billing_accounts.locators.DETAILS_TAB.click()
            self.billing_accounts.locators.DETAIL.wait_to_be_visible()
            self.billing_accounts.locators.DETAIL_ADJUSTED[0].wait_to_have_text(
                f"{detail_adjusted + adjustment_sum:.2f}"
            )

        with allure.step("Перейти на вкладку 'Счета-фактуры', проверить что сумма корректировки учтена"):
            self.billing_accounts.locators.INVOICES_TAB.click()
            self.billing_accounts.locators.INVOICE.wait_to_be_visible()
            self.billing_accounts.locators.INVOICE_TYPE.wait_for_text_in_all([tax_invoice_type])
            tax_invoice_index = self.billing_accounts.locators.INVOICE_TYPE.text_list.index(tax_invoice_type)
            self.billing_accounts.locators.INVOICE_ADJUSTED[tax_invoice_index].wait_to_have_text(
                f"{tax_invoice_adjusted + adjustment_sum:.2f}"
            )
